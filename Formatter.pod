...
module, and call the C<format> constructor method on it, with the text
string as parameter. You may then call either the C<document> or
C<fragment> method to get HTML returned.
...
argument it must take a string with the text that one wants converted.
...
as an array where each element is a hash, with keys C<url> and
...
title can be found, the title key should have an empty string.
